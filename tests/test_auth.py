...
def test_register_user_success(client: TestClient, db_session: Session, test_user_data: dict):
    ...
    user_in_db = db_session.query(models.User).filter(models.User.email == test_user_data["email"]).first()
    assert user_in_db is not None

def test_register_duplicate_email(client: TestClient, test_user):
    response = client.post("/api/auth/register", json={
        "username": "anotheruser", "email": "test@example.com", "password": "password",
...
def test_forgot_password_flow(client: TestClient, test_user, test_user_data: dict):
    ...
    response_verify_a = client.post("/api/auth/forgot-password/verify-answers", json={
        "identifier": test_user_data["email"],
    ...
    response_set_pw = client.post("/api/auth/forgot-password/set-new-password", headers=reset_headers, json={
        "new_password": new_password
    })
