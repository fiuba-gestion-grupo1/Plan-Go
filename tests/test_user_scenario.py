...
def simulate_user_scenario():
    ...
    response = requests.post(
        f"{BASE_URL}/api/itineraries/request", json=itinerary_data, headers=headers
    )
    ...
    if response.status_code == 200:
        ...
        if "17" in itinerary_text and (
            "DÍA 1" in itinerary_text or "día 1" in itinerary_text
        ):
            print(f"✅ El día 17 aparece en el primer día del itinerario")
        elif "16" in itinerary_text and (
            "DÍA 1" in itinerary_text or "día 1" in itinerary_text
        ):
