...
def test_ai_validation_full():
    ...
    print(
        f"   📊 Indicadores de validación: {len(found_validation)}/{len(validation_indicators)}"
    )
    ...
    if utilization_info:
        print(
            f"   ✅ Utilización presupuesto: {utilization_info[0].split(':')[1].strip()}"
        )

    if validation_score >= 8:
        print(f"\n🌟 EXCELENTE: Sistema de validación funcionando perfectamente!")
        print(
            f"   🔥 Validación completa con {validation_score} indicadores detectados"
        )
