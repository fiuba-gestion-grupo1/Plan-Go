...
def test_paste_endpoint():
    ...
    list_response = requests.get(f"{BASE_URL}/api/itineraries/ai-list", headers=headers)
    ...
    if itineraries:
        ...
        if convert_response.status_code == 200:
            ...
            print(
                f"   🏛️ Publicaciones: {len(conversion_result.get('publications_used', []))}"
            )
    ...
    if total_itineraries > 0:
        quality_score += 25
    if itineraries and all("preview" in it for it in itineraries):
        quality_score += 25
    if convert_response.status_code == 200:
        quality_score += 25
