...
def get_my_trips(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    participant_links = (
        db.query(models.TripParticipant).filter_by(user_id=user.id).all()
    )
...
def create_trip(
    ...
    if not name:
        raise HTTPException(
            status_code=400, detail="El nombre del viaje es obligatorio"
        )
...
def invite_user_to_trip(
    ...
    if not username_to_invite:
        raise HTTPException(
            status_code=400, detail="Debe indicar el nombre de usuario a invitar"
        )
    ...
    inviter_participant = (
        db.query(models.TripParticipant)
        .filter_by(trip_id=trip_id, user_id=user.id)
        .first()
    )
    ...
    if invited_user.role != "premium":
        raise HTTPException(
            status_code=403, detail="Solo se pueden invitar usuarios premium"
        )

    already_participant = (
        db.query(models.TripParticipant)
        .filter_by(trip_id=trip_id, user_id=invited_user.id)
        .first()
    )
    if already_participant:
        raise HTTPException(
            status_code=400, detail="El usuario ya es participante de este viaje"
        )

    any_invitation = (
        db.query(models.TripInvitation)
        .filter_by(trip_id=trip_id, invited_user_id=invited_user.id)
        .first()
    )
    if any_invitation:
        raise HTTPException(
            status_code=400,
            detail="Ya existe una invitación registrada para este usuario en este viaje",
        )
...
def respond_to_invitation(
    ...
    invitation = (
        db.query(models.TripInvitation)
        .filter_by(id=invitation_id, invited_user_id=user.id)
        .first()
    )
    if not invitation:
    ...
    if invitation.status != "pending":
        raise HTTPException(
            status_code=400, detail="Esta invitación ya fue respondida previamente"
        )
    ...
    if action == "accept":
        ...
        existing_participant = (
            db.query(models.TripParticipant)
            .filter_by(trip_id=invitation.trip_id, user_id=user.id)
            .first()
        )
...
def update_trip(
    ...
    if trip.user_id != user.id:
        ...
        if not is_participant:
            raise HTTPException(
                status_code=403, detail="No sos participante de este viaje"
            )
...
def delete_trip(
    trip_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)
):
    trip = db.query(models.Trip).filter_by(id=trip_id).first()
    if not trip:
        raise HTTPException(status_code=404, detail="Viaje no encontrado")
    ...
    db.query(models.TripParticipant).filter(
        models.TripParticipant.trip_id == trip_id
    ).delete()
    db.query(models.TripInvitation).filter(
...
def add_expense(
    ...
    try:
        ...
        if amount_val < 0:
            raise HTTPException(
                status_code=400, detail="El monto del gasto no puede ser negativo."
            )
    except (ValueError, TypeError):
        raise HTTPException(
            status_code=400, detail="El monto debe ser un número válido."
        )
...
def export_trip_expenses(
    ...
    for e in expenses:
        data.append(
            [e.date.strftime("%d/%m/%Y"), e.name, e.category, f"{e.amount:.2f}"]
        )
        total += e.amount
    ...
    return FileResponse(
        tmpfile.name, filename=f"Gastos_{trip.name}.pdf", media_type="application/pdf"
    )
...
def join_trip(
    ...
    if user.role != "premium":
        raise HTTPException(
            status_code=403, detail="Solo los usuarios premium pueden unirse a un viaje"
        )
...
def calculate_balances(
    ...
    if not participants:
        raise HTTPException(
            status_code=400, detail="No hay participantes en este viaje"
        )
    ...
    for p in participants:
        user_gastos = (
            db.query(models.Expense).filter_by(trip_id=trip_id, user_id=p.user_id).all()
        )
    ...
    for p in participants:
        ...
        balances.append(
            {"username": username, "pagado": pagado, "debe_o_recibe": balance}
        )
