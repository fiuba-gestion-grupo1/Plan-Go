...
def require_premium(
    ...
    if current_user.role != "premium":
        raise HTTPException(
            status_code=403, detail="Solo usuarios premium pueden publicar reseñas"
        )
    return current_user
...
def _get_or_create_category(
    db: Session, slug: str, name: Optional[str] = None
) -> models.Category:
...
@router.post(
    "", response_model=schemas.PublicationOut, status_code=status.HTTP_201_CREATED
)
def create_publication(
    ...
    for f in files:
        if f.content_type not in ("image/jpeg", "image/png", "image/webp"):
            raise HTTPException(
                status_code=400, detail="Formato de imagen inválido (usa JPG/PNG/WebP)"
            )
    ...
    if categories:
        slugs = [
            _normalize_slug(s) for s in categories.split(",") if _normalize_slug(s)
        ]
        for slug in slugs:
...
def submit_publication(
    ...
    pub = models.Publication(
        ...
        activities=(
            activities_list if hasattr(models.Publication, "activities") else None
        ),
        cost_per_day=(
            cost_per_day if hasattr(models.Publication, "cost_per_day") else None
        ),
        duration_min=(
            duration_min if hasattr(models.Publication, "duration_min") else None
        ),
        available_days=(
            available_days_list
            if hasattr(models.Publication, "available_days")
            else None
        ),
        available_hours=(
            available_hours_list
            if hasattr(models.Publication, "available_hours")
            else None
        ),
    )
    ...
    return schemas.PublicationOut(
        ...
        **(
            {"rating_avg": getattr(pub, "rating_avg", 0.0) or 0.0}
            if hasattr(schemas.PublicationOut, "model_fields")
            or hasattr(schemas.PublicationOut, "__fields__")
            else {}
        ),
    )
...
def list_my_submissions(
    ...
    pubs = (
        db.query(models.Publication)
        .filter(models.Publication.created_by_user_id == current_user.id)
        .order_by(models.Publication.created_at.desc())
        .all()
    )
    ...
    if destination and len(destination.strip()) >= 2:
        ...
        for p in pubs:
            ...
            for field in location_fields:
                ...
                if (
                    normalized_dest_lower in field_lower
                    or normalized_dest_no_accents in field_no_accents
                ):
                    match = True
                    break
        ...
        for p in pubs:
            ...
            for field in search_fields:
                ...
                if (
                    normalized_q_lower in field_lower
                    or normalized_q_no_accents in field_no_accents
                ):
                    match = True
                    break
...
def list_publications_public(
    category: Optional[str] = Query(
        None, description="Slugs separados por coma, ej: aventura,cultura"
    ),
    db: Session = Depends(get_db),
    ...
    if category:
        ...
        if slugs:
            q = (
                q.join(models.Publication.categories)
...
def list_reviews(
    ...
    rows = (
        ...
        .filter(models.Review.status.in_(["approved", "under_review"]))
        .options(
            selectinload(models.Review.comments).selectinload(
                models.ReviewComment.author
            )
        )
...
def toggle_review_like(
    ...
    new_count = (
        db.query(func.count(models.ReviewLike.id))
        .filter(models.ReviewLike.review_id == review_id)
        .scalar()
    )
...
@router.post(
    ...
    response_model=schemas.ReviewCommentOut,
    status_code=status.HTTP_201_CREATED,
)
def create_review_comment(
    ...
    return schemas.ReviewCommentOut(
        ...
        author_username=user.username,
        created_at=(
            new_comment.created_at.strftime("%Y-%m-%d %H:%M:%S")
...
def approve_publication(
    pub_id: int, db: Session = Depends(get_db), _: models.User = Depends(require_admin)
):
    pub = db.query(models.Publication).filter(models.Publication.id == pub_id).first()
    if not pub:
        raise HTTPException(status_code=404, detail="Publicación no encontrada")
...
def request_deletion(
    pub_id: int,
    req: schemas.DeletionRequestCreate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user),
):
    """
    ...
    """
    print(
        f"[DEBUG] request_deletion called: pub_id={pub_id}, reason={req.reason}, user_id={current_user.id}"
    )
    pub = db.query(models.Publication).filter(models.Publication.id == pub_id).first()
    if not pub:
        raise HTTPException(status_code=404, detail="Publicación no encontrada")
    ...
    if existing_request:
        raise HTTPException(
            status_code=400,
            detail="Ya existe una solicitud de eliminación pendiente para esta publicación",
        )

    new_request = models.DeletionRequest(
        publication_id=pub_id, requested_by_user_id=current_user.id, reason=req.reason
    )
    db.add(new_request)
    db.commit()

    return {
        "message": "Solicitud de eliminación enviada. Será revisada por un administrador."
    }


@router.get(
    "/deletion-requests/pending", response_model=List[schemas.DeletionRequestOut]
)
...
def approve_deletion_request(
    ...
    req = (
        db.query(models.DeletionRequest)
        .filter(models.DeletionRequest.id == request_id)
        .first()
    )
    if not req:
...
@router.post(
    "/{pub_id}/reviews/{review_id}/report",
    response_model=schemas.ReviewReportOut,
    status_code=status.HTTP_201_CREATED,
)
def report_review(
    pub_id: int,
    ...
    review = (
        ...
        .filter(models.Review.id == review_id, models.Review.publication_id == pub_id)
        .first()
    )
    ...
    if review.author_id == current_user.id:
        raise HTTPException(
            status_code=400, detail="No puedes reportar tu propia reseña"
        )
    ...
    report_with_data = (
        ...
        .options(
            ...
            selectinload(models.ReviewReport.review).selectinload(
                models.Review.publication
            ),
            selectinload(models.ReviewReport.reporter),
        )
        .filter(models.ReviewReport.id == report.id)
        .first()
    )
...
def list_visited(
    ...
    logger.debug(f"[visited] target_user_id={target_user_id}, count={len(visited)}")
