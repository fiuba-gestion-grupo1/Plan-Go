...
class ItineraryValidator:
    ...
    def validate_itinerary(
        ...
        if "used_publications" in itinerary_data:
            return self._validate_ai_itinerary(
                itinerary_data, budget, cant_persons, start_date, end_date
            )
        else:
            return self._validate_custom_itinerary(
                itinerary_data, budget, cant_persons, start_date, end_date
            )

    def _validate_ai_itinerary(
        self,
        ai_data: dict,
        budget: float,
        cant_persons: int,
        ...
        for pub_usage in used_publications:
            ...
            publication = (
                self.db.query(models.Publication)
            ...
            real_cost = self._calculate_real_cost(
                publication, times_used, cant_persons, len(days_used)
            )
        ...
        return {
            ...
            "budget_utilization_percent": (
                (real_total_cost / budget) * 100 if budget > 0 else 0
            ),
            "validated_publications": validated_publications,
            "validation_summary": self._generate_validation_summary(),
        }

    def _validate_custom_itinerary(
        self,
        custom_data: dict,
        budget: float,
        cant_persons: int,
        ...
        for pub_id, usage_data in publication_usage.items():
            ...
            self._validate_publication_availability(
                publication, days_used, hours_used, pub_id
            )
    ...
    def _validate_publication_availability(
        self,
        ...
        return {"valid": len(availability_errors) == 0, "errors": availability_errors}
    ...
    def _time_in_available_ranges(
        self, time_slot: str, available_ranges: List[str]
    ) -> bool:
    ...
    def _validate_dates(
        self, start_date: str, end_date: str, used_publications: List[dict]
    ):
        ...
        try:
            ...
            self._add_error(
                "INVALID_DATE_FORMAT", "Formato de fechas de inicio o fin inválido"
            )
    ...
    def _add_warning(self, warning_type: str, message: str, **kwargs):
        ...
        warning = ItineraryValidationError(
            warning_type, message, severity="warning", **kwargs
        )
