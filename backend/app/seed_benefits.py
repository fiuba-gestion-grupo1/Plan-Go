...
def create_premium_benefits(db: Session):
    ...
    restaurants = (
        ...
        .join(Publication.categories)
        .filter(models.Category.slug == "gastronomia")
        .all()
    )
    ...
    hotels = (
        db.query(Publication)
        .join(Publication.categories)
        .filter(models.Category.slug == "hotel")
        .all()
    )
    ...
    attractions = (
        db.query(Publication)
        .join(Publication.categories)
        .filter(models.Category.slug.in_(["actividad", "aventura", "cultura"]))
        .all()
    )
    ...
    for i, restaurant in enumerate(restaurants):
        ...
        existing = (
            db.query(PremiumBenefit)
            .filter(PremiumBenefit.publication_id == restaurant.id)
            .first()
        )

        if not existing:
            benefit = PremiumBenefit(publication_id=restaurant.id, **benefit_data)
            db.add(benefit)
            benefit_count += 1
    ...
    for i, hotel in enumerate(hotels):
        ...
        existing = (
            db.query(PremiumBenefit)
            .filter(PremiumBenefit.publication_id == hotel.id)
            .first()
        )

        if not existing:
            benefit = PremiumBenefit(publication_id=hotel.id, **benefit_data)
            db.add(benefit)
            benefit_count += 1
    ...
    for i, attraction in enumerate(attractions):
        ...
        existing = (
            db.query(PremiumBenefit)
            .filter(PremiumBenefit.publication_id == attraction.id)
            .first()
        )

        if not existing:
            benefit = PremiumBenefit(publication_id=attraction.id, **benefit_data)
            db.add(benefit)
            benefit_count += 1
...
def add_premium_publications(db: Session):
    ...
    new_publications = [
        {
            ...
            "reviews": [
                (
                    5,
                    "Vista increíble de Puerto Madero. La cena fue perfecta para nuestro aniversario.",
                ),
                (
                    4,
                    "Ambiente elegante y comida exquisita. El servicio podría ser un poco más rápido.",
                ),
                (
                    5,
                    "El mejor lugar para una cita especial. Los cócktails son arte líquido.",
                ),
                (
                    5,
                    "Calidad-precio excelente considerando la ubicación y la vista panorámica.",
                ),
            ],
        },
        {
            ...
            "reviews": [
                (
                    5,
                    "El sommelier conoce cada etiqueta de memoria. Los maridajes son perfectos.",
                ),
                (
                    5,
                    "Gran variedad de vinos locales e internacionales. Tapas deliciosas.",
                ),
                (
                    4,
                    "Ambiente acogedor y selección impresionante. Los precios son razonables para la calidad.",
                ),
                (
                    5,
                    "Imperdible para los amantes del vino. Aprendí mucho sobre los terroirs mendocinos.",
                ),
            ],
        },
        {
            ...
            "reviews": [
                (
                    5,
                    "El mejor bife de chorizo que probé en años. La parrilla se ve desde la mesa, espectáculo incluido.",
                ),
                (
                    4,
                    "Tradicional y auténtico. Las empanadas de entrada son caseras y deliciosas.",
                ),
                (
                    5,
                    "Ambiente familiar perfecto para domingo. El asador es un artista con las brasas.",
                ),
                (
                    5,
                    "Carnes de primera calidad y cocción perfecta. El chimichurri casero está brutal.",
                ),
            ],
        },
        {
            ...
            "reviews": [
                (
                    5,
                    "Perfecto para trabajar con laptop. WiFi excelente y el café está buenísimo.",
                ),
                (
                    4,
                    "Las exposiciones cambian cada mes. Me encanta el ambiente bohemio y tranquilo.",
                ),
                (
                    5,
                    "Los tostados artesanales son increíbles. Apoyan mucho al arte local.",
                ),
                (
                    4,
                    "Ideal para reuniones informales. La decoración con obras locales le da un toque único.",
                ),
            ],
        },
        {
            ...
            "reviews": [
                (
                    5,
                    "Lujo absoluto en el corazón de Recoleta. El spa es de otro nivel, relajación total.",
                ),
                (
                    5,
                    "Servicio impecable, me sentí como VIP desde el check-in. La piscina climatizada en el rooftop es increíble.",
                ),
                (
                    4,
                    "Habitaciones amplias y elegantes. El desayuno buffet tiene opciones para todos los gustos.",
                ),
                (
                    5,
                    "Ubicación perfecta para explorar museos y shopping. El concierge me organizó todo el itinerario.",
                ),
            ],
        },
        {
            ...
            "reviews": [
                (
                    5,
                    "Decoración auténtica con arte regional salteño. Cada habitación cuenta una historia.",
                ),
                (
                    4,
                    "Ubicación excelente para recorrer el centro histórico a pie. Personal muy amable y local.",
                ),
                (
                    5,
                    "El desayuno incluye productos regionales deliciosos. Se siente la calidez del norte argentino.",
                ),
                (
                    5,
                    "Hotel pequeño pero con gran atención personalizada. Conocen todos los tours y excursiones.",
                ),
            ],
        },
        {
            ...
            "reviews": [
                (
                    5,
                    "Despertar con vista a las montañas nevadas no tiene precio. Resort familiar pero también romántico.",
                ),
                (
                    5,
                    "Las cabañas son amplias y acogedoras. Chimenea a leña y todo el confort moderno.",
                ),
                (
                    4,
                    "Perfecto para desconectar de la ciudad. Las actividades para niños están muy bien organizadas.",
                ),
                (
                    5,
                    "El restaurante del resort ofrece platos regionales con ingredientes locales. Vista espectacular desde el comedor.",
                ),
            ],
        },
        {
            ...
            "reviews": [
                (
                    5,
                    "Los niños se divirtieron tanto que no se querían ir. Aprendieron jugando, una maravilla.",
                ),
                (
                    5,
                    "El planetario es increíble, te sentís viajando por el espacio. Exhibiciones muy modernas.",
                ),
                (
                    4,
                    "Perfecto para pasar una tarde en familia. Los talleres están muy bien diseñados.",
                ),
                (
                    5,
                    "Interactivo de verdad, no solo para mirar. Los experimentos de física son geniales.",
                ),
            ],
        },
        {
            ...
            "reviews": [
                (
                    5,
                    "¡Adrenalina pura! La tirolesa sobre el lago es espectacular. Súper seguro y organizado.",
                ),
                (
                    5,
                    "Actividades para toda la familia, desde niños hasta abuelos. Los instructores muy profesionales.",
                ),
                (
                    4,
                    "Pasamos todo el día ahí. El circuito de escalada en árboles está genial, se siente la naturaleza.",
                ),
                (
                    5,
                    "Precios accesibles para toda la diversión que ofrecen. Volveremos seguro.",
                ),
            ],
        },
        {
            ...
            "reviews": [
                (
                    5,
                    "Conocimos rincones gastronómicos que jamás hubiésemos encontrado solos. El chef guía es un genio.",
                ),
                (
                    5,
                    "La experiencia incluye 5 paradas con degustaciones. Valor increíble por lo que está incluido.",
                ),
                (
                    4,
                    "Aprendés mucho sobre la historia culinaria del barrio mientras comés delicioso. Muy recomendable.",
                ),
                (
                    5,
                    "Tour pequeño y personalizado. Se siente exclusivo y no masivo como otros tours.",
                ),
            ],
        },
    ]
    ...
    for pub_data in new_publications:
        existing = (
            db.query(Publication)
            .filter(Publication.place_name == pub_data["place_name"])
            .first()
        )
        ...
        if not existing:
            ...
            publication = Publication(
                ...
                street=(
                    pub_data["address"].split(",")[0] if pub_data.get("address") else ""
                ),
            ...
            if reviews_data:
                ...
                for i, (rating, comment) in enumerate(reviews_data):
                    ...
                    review = Review(
                        ...
                        comment=comment,
                        created_at=datetime.utcnow()
                        - timedelta(days=len(reviews_data) - i),
                    )
                    db.add(review)
...
def main():
    ...
    try:
        ...
        print(
            "Los usuarios premium ahora pueden disfrutar de descuentos y beneficios exclusivos."
        )
