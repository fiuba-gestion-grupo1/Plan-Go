...
export default function ItineraryRequest({ initialView = 'form' }) {
    ...
    async function handleDeleteItinerary(itineraryId) {
        ...
        try {
            ...
            // Si estamos viendo el itinerario eliminado, volver a la lista
                                    ...
                                    <div className="card shadow-sm h-100 border-success">
                                                ...
                                                <div className="flex-grow-1">
                                                    ...
                                                    <div className="mt-2 d-flex flex-wrap gap-2">
                                                        <span className="badge bg-light text-dark border">
                                                            ⭐ {Number(p.rating_avg).toFixed(1)} • {p.rating_count} reseña{p.rating_count === 1 ? "" : "s"}
                                                        </span>
                                                        {(p.categories || []).map((c) => (
                                        ...
                                        {p.photos?.length ? (
                                            <div id={`itin-carousel-${p.id}`} className="carousel slide" data-bs-ride="false">
                                                ...
                                                {p.photos.length > 1 && (
                                                    <>
                                                        <button className="carousel-control-prev" type="button" data-bs-target={`#itin-carousel-${p.id}`} data-bs-slide="prev" style={{ filter: "drop-shadow(0 0 6px rgba(0,0,0,.4))" }}>
                                                            <span className="carousel-control-prev-icon" />
                                                        </button>
                                                        <button className="carousel-control-next" type="button" data-bs-target={`#itin-carousel-${p.id}`} data-bs-slide="next" style={{ filter: "drop-shadow(0 0 6px rgba(0,0,0,.4))" }}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-4">
                ...
                {myItineraries.length > 0 && (
                    <button 
                        ...
                        onClick={handleViewItineraries}
                    >
                        Ver mis itinerarios ({myItineraries.length})
                    </button>
                )}
            </div>
