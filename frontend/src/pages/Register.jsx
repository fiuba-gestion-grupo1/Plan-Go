...
export default function Register({ setView }) {
  ...
  useEffect(() => {
    ...
    if (invitationCode) {
      ...
      setInvitationInfo({
        code: invitationCode,
        message:
          "¡Te han invitado a unirte a Plan&Go! Completa el registro para obtener tu cuenta.",
      });
    }
  }, []);
  ...
  const available_q2 = SECURITY_QUESTIONS.filter(
    (q) => q !== formData.security_question_1,
  );
  const available_q1 = SECURITY_QUESTIONS.filter(
    (q) => q !== formData.security_question_2,
  );
